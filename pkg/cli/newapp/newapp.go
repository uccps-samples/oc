...
var (
	...
	newAppExample = templates.Examples(`
		...
		oc new-app centos/ruby-25-centos7~https://github.com/uccps-samples/ruby-ex.git
...
func TransformRunError(err error, commandPath string, groups ErrorGroups, config *newcmd.AppConfig) {
		...
		for i, match := range t.Matches {
			...
			if i >= 5 {
				groups.Add(
					"multiple-matches",
					heredoc.Docf(`
						The argument %[1]q could apply to the following container images, Uccp image streams, or templates:
	...
	case newappapp.ErrPartialMatch:
		...
		groups.Add(
			"partial-match",
			heredoc.Docf(`
					The argument %[1]q only partially matched the following container image, Uccp image stream, or template:
