...
func NewMustGatherCommand(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().StringSliceVar(&o.Images, "image", o.Images, "Specify a must-gather plugin image to run. If not specified, Uccp's default must-gather image will be used.")
...
func (o *MustGatherOptions) completeImages() error {
	...
	if len(o.Images) == 0 {
		var image string
		var err error
		if image, err = o.resolveImageStreamTag("uccp", "must-gather", "latest"); err != nil {
			o.log("%v\n", err)
