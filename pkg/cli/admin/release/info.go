...
func NewInfo(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: templates.LongDesc(`
			Show information about an Uccp release.

			This command retrieves, verifies, and formats the information describing an Uccp update.
...
func replaceClusterSemanticArgs(f kcmdutil.Factory, args []string, semanticArgs map[string]semver.Version) ([]string, error) {
	...
	if err != nil {
		return args, fmt.Errorf("info expects one argument, or a connection to an Uccp 1.x server: %v", err)
	}
	...
	if err != nil {
		if errors.IsNotFound(err) || errors.ReasonForError(err) == metav1.StatusReasonUnknown {
			...
			return args, fmt.Errorf("info expects one argument, or a connection to an Uccp 1.x server")
		}
		return args, fmt.Errorf("info expects one argument, or a connection to an Uccp 1.x server: %v", err)
	}
