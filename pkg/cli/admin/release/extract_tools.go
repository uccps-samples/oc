...
var (
	...
	The Uccp installer \u0060utccp-install\u0060 makes it easy to get a cluster
	running on the public cloud or your local infrastructure.

	## Installing the tools

	After extracting this archive, you can move the \u0060utccp-install\u0060 binary
	to a location on your PATH such as \u0060/usr/local/bin\u0060, or keep it in a temporary
	directory and reference it via \u0060./utccp-install\u0060.
	`)
...
func (o *ExtractOptions) extractCommand(command string) error {
	...
	availableTargets := []extractTarget{
		{
			...
			Arch:    "amd64",
			Command: "oc",
			Mapping: extract.Mapping{Image: "cli-artifacts", From: "usr/share/uccp/mac/oc"},
		...
		},
		{
			...
			Command: "oc",
			NewArch: true,
			Mapping: extract.Mapping{Image: "cli-artifacts", From: "usr/share/uccp/mac_arm64/oc"},

			LinkTo:               []string{"kubectl"},
			Readme:               readmeCLIUnix,
			InjectReleaseVersion: true,
			ArchiveFormat:        "uccp-client-mac-arm64-%s.tar.gz",
		},
		{
			OS:      "linux",
		...
		},
		{
			...
			Arch:    "amd64",
			Command: "oc",
			Mapping: extract.Mapping{Image: "cli-artifacts", From: "usr/share/uccp/linux_amd64/oc"},
		...
		},
		{
			...
			Arch:    "amd64",
			Command: "oc",
			Mapping: extract.Mapping{Image: "cli-artifacts", From: "usr/share/uccp/windows/oc.exe"},
		...
		},
		{
			...
			Mapping: extract.Mapping{Image: "installer-artifacts", From: "usr/share/uccp/mac/uccp-install"},
		...
		},
		{
			...
			Command: "uccp-install",
			NewArch: true,
			Mapping: extract.Mapping{Image: "installer-artifacts", From: "usr/share/uccp/mac_arm64/uccp-install"},

			Readme:               readmeInstallUnix,
			InjectReleaseImage:   true,
			InjectReleaseVersion: true,
			ArchiveFormat:        "uccp-install-mac-arm64-%s.tar.gz",
		},
		{
			OS:      "linux",
			Arch:    targetReleaseArch,
		...
		},
		{
			...
			Command: "uccp-install",
			NewArch: true,
			Mapping: extract.Mapping{Image: "installer-artifacts", From: "usr/share/uccp/linux_amd64/uccp-install"},
	...
	if willArchive {
		...
		fmt.Fprintf(buf, heredoc.Doc(`
			...
			The utccp-install binary has been preconfigured to install the following release:
