...
func NewMirror(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: templates.LongDesc(`
			Mirror an Uccp release image to another registry and produce a configuration
			...
			The common use for this command is to mirror a specific Uccp release version to
			...
			target repository and then prints the correct information to give to Uccp to use
			that content offline. An alternate mode is to specify --to-image-stream, which imports
			the images directly into an Uccp image stream.
