...
func NewInstallError(script string) error {
	return Error{
		...
		Suggestion: fmt.Sprintf("set the scripts URL parameter with the location of the S2I scripts, or check if the image has the %q label set", "io.uccp.s2i.scripts-url"),
	}
}
...
func NewAssembleUserNotAllowedError(image string, usesConfig bool) error {
	...
	if usesConfig {
		...
		msg = fmt.Sprintf("image %q includes the %q label whose value is not within the allowed range", image, "io.uccp.s2i.assemble-user")
		suggestion = fmt.Sprintf("modify the %q label in image %q to use a numeric user within the allowed range, or build without the allowed UIDs configuration set", "io.uccp.s2i.assemble-user", image)
	}
	return Error{
		Message:    msg,
		ErrorCode:  UserNotAllowedError,
