...
func TestNewAppAddArguments(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"source": {
			args:    []string{".", testDir, "https://github.com/uccps-samples/origin.git"},
			repos:   []string{".", testDir, "https://github.com/uccps-samples/origin.git"},
			unknown: []string{},
		},
		"source custom ref": {
		...
		"mix 1": {
			args:       []string{"https://github.com/uccps-samples/origin.git", "mysql+ruby~git@github.com/uccps-samples/origin.git", "env1=test", "ruby-helloworld-sample"},
			repos:      []string{"https://github.com/uccps-samples/origin.git"},
	...
	tests := []struct {
			...
			config: &cmd.AppConfig{
				ComponentInputs: cmd.ComponentInputs{
					SourceRepositories: []string{
						...
						"https://github.com/uccps-samples/django-ex",
					},
				},
				GenerationInputs: cmd.GenerationInputs{
					Dockerfile: "FROM quay.io/centos7/ruby-27-centos7\nRUN false",
		...
		},
		{
			...
			config: &cmd.AppConfig{
				ComponentInputs: cmd.ComponentInputs{
					SourceRepositories: []string{
						"https://github.com/uccps-samples/nodejs-ex",
					},
				},
				GenerationInputs: cmd.GenerationInputs{
			...
			checkResult: func(res *cmd.AppResult) error {
				...
				if !reflect.DeepEqual(bc.Spec.Source, buildv1.BuildSource{
					ContextDir: "openshift/pipeline",
					Git:        &buildv1.GitBuildSource{URI: "https://github.com/uccps-samples/nodejs-ex"},
		...
		},
		{
			...
			config: &cmd.AppConfig{
				ComponentInputs: cmd.ComponentInputs{
					Components: []string{
						"registry.centos.org/centos/nodejs-12-centos7~https://github.com/uccps-samples/nodejs-ex",
					},
				},
				GenerationInputs: cmd.GenerationInputs{
					ContextDir: "openshift/pipeline",
