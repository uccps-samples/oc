...
The argument %[1]q only partially matched the following container image or Uccp image stream:
...
func (e ErrNoTagsFound) Suggestion(commandName string) string {
	...
	return fmt.Sprintf(`%[3]s
The argument %[1]q matched the following Uccp image stream which has no tags:
