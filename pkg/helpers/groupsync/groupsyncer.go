...
func (s *LDAPGroupSyncer) Sync() ([]*userv1.Group, []error) {
	...
	for _, ldapGroupUID := range ldapGroupUIDs {
		...
		klog.V(1).Infof("Has Uccp users %v", usernames)
		...
		if err != nil {
			...
			fmt.Fprintf(s.Err, "Error building Uccp group for LDAP group %q: %v.\n", ldapGroupUID, err)
			errors = append(errors, err)
			continue
		}
		...
		if !s.DryRun {
			...
			if err := s.updateOpenShiftGroup(openshiftGroup); err != nil {
				fmt.Fprintf(s.Err, "Error updating Uccp group %q for LDAP group %q: %v.\n", openshiftGroup.Name, ldapGroupUID, err)
				errors = append(errors, err)
				continue
			}
		}
...
func (s *LDAPGroupSyncer) determineUsernames(members []*ldap.Entry) ([]string, error) {
	...
	for _, member := range members {
		...
		klog.V(2).Infof("Found Uccp username %q for LDAP user for %v", username, member)
